  ...
  private static int[] defensas=new int[100];
    ...
    public void lista(){
        ...
        System.out.println("***Listado de los personajes***");
         System.out.printf("%-5s %-20s %-20s %-10s %-10s %-10s %-10s %-10s\n","ID", "Nombre", "objeto", "HP","ataque","velocidad","agilidad","defensa");
        for (int a = 0; a < Cantidadpersonajes; a++) {
        
        System.out.printf("%-5s %-20s %-20s %-10s %-10s %-10s %-10s %-10s\n",(a + 1), nombres[a],objetos[a],healtpoints[a],ataques[a],velocidades[a],agilidades[a],defensas[a]);
        }
    }
    ...
    public String buscarID(int Val,int ID){
        ...
        System.out.println("***Personaje seleccionado***");
         System.out.printf("%-5s %-20s %-20s %-10s %-10s %-10s %-10s %-10s\n","ID", "Nombre", "objeto", "HP","ataque","velocidad","agilidad","defensa");
        System.out.printf("%-5s %-20s %-20s %-10s %-10s %-10s %-10s %-10s\n",(ID), nombres[lista],objetos[lista],healtpoints[lista],ataques[lista],velocidades[lista],agilidades[lista],defensas[lista]);
    ...
    public boolean buscarnombre(int Val,String nombre){
       for (int i = 0; i < Cantidadpersonajes; i++) {
            if (nombres[i].equalsIgnoreCase(nombre)) {
                ...
                int validacion=Val;
                System.out.printf("%-5s %-20s %-20s %-10s %-10s %-10s %-10s %-10s\n","ID", "Nombre", "objeto", "HP","ataque","velocidad","agilidad","defensa");
                System.out.printf("%-5s %-20s %-20s %-10s %-10s %-10s %-10s %-10s\n",(i+1), nombres[i],objetos[i],healtpoints[i],ataques[i],velocidades[i],agilidades[i],defensas[i]);
               if(validacion==1){
    ...
    System.out.printf("%-5s %-20s %-20s %-10s %-10s %-10s %-10s %-10s\n","ID", "Nombre", "objeto", "HP","ataque","velocidad","agilidad","defensa");
    System.out.printf("%-5s %-20s %-20s %-10s %-10s %-10s %-10s %-10s\n",(editar+1), nombres[editar],objetos[editar],healtpoints[editar],ataques[editar],velocidades[editar],agilidades[editar],defensas[editar]);
    return true;
    }
    ...
    public String buscarbid(int id1, int id2){
        ...
        InicioBatalla(id1,id2,nombres[lista1],nombres[lista2],objetos[lista1],objetos[lista2],healtpoints[lista1],healtpoints[lista2],ataques[lista1],ataques[lista2],velocidades[lista1],velocidades[lista2],agilidades[lista1],agilidades[lista2],defensas[lista1],defensas[lista2]);
        }
      return null;
  }
    public boolean buscarbnom(String nom1,String nom2){
         ...
         if (id1 != -1 && id2 != -1) {
          revisionnmob(id1, id2, nombres, objetos, healtpoints, ataques, velocidades, agilidades, defensas);
          } else {
    ...
    public static void revisionnmob(int id1, int id2,String[] nombres, String[] objetos, int[] healtpoints, int[] ataques,int[] velocidades, int[] agilidades, int[] defensas) {

    System.out.printf("%-5s %-20s %-20s %-10s %-10s %-10s %-10s %-10s\n","ID", "Nombre", "objeto", "HP", "ataque", "velocidad", "agilidad", "defensa");

    System.out.printf("%-5s %-20s %-20s %-10s %-10s %-10s %-10s %-10s\n",(id1 + 1), nombres[id1], objetos[id1], healtpoints[id1],ataques[id1], velocidades[id1], agilidades[id1], defensas[id1]);

    System.out.printf("%-5s %-20s %-20s %-10s %-10s %-10s %-10s %-10s\n",(id2 + 1), nombres[id2], objetos[id2], healtpoints[id2],ataques[id2], velocidades[id2], agilidades[id2], defensas[id2]);
